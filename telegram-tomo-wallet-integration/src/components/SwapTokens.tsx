...
import { ISwapTokenType } from '@tomo-inc/tomo-telegram-sdk/dist/hooks/useSwapTokens';
import * as React from 'react';
import { useMemo, useState } from 'react';
import { Hex, erc20Abi, formatUnits, parseUnits, zeroAddress } from 'viem';
import { useReadContract } from 'wagmi';
import { useLoading } from '../hooks/useLoading';

const SwapTokens = () => {
  const { swapChainIds, getChain, getChainId } = useChains();
  ...
  const { tokens: fromTokens } = useSwapTokens({
    chain: fromChain,
    content: fromContext,
  });
  ...
  const { tokens: toTokens } = useSwapTokens({
    chain: toChain,
    content: toContext,
  });
  ...
  return (
    <div>
      <div className={'module-title'}>SwapTokens</div>
      <h3>fromInfo</h3>
      <p>
        fromChain:
        <select
